...
class ParticleSimulation(Scene):
    ...
    def construct(self):
        ...
        velocities = [UP * i + RIGHT * j for i, j in
                      zip(np.random.uniform(-0.2, 0.2, num_particles), np.random.uniform(-0.2, 0.2, num_particles))]
        positions = []
...
class ParticleSimulationOptimized(Scene):
    ...
    def construct(self):
        ...
        velocities = [UP * i + RIGHT * j for i, j in
                      zip(np.random.uniform(-0.3, 0.3, num_particles), np.random.uniform(-0.3, 0.3, num_particles))]
        positions = []
        start = -2.5
        end = 3.1
...
class ParticleInBox(Scene):
    ...
    def construct(self):
        ...
        end_vel = start_vel + (ball.velocity / np.linalg.norm(ball.velocity)) * np.linalg.norm(
            ball.velocity) / np.linalg.norm(ball.acceleration)
        ...
        v_label = VGroup(left, v_x, comma, v_y, right).arrange_submobjects(RIGHT, buff=SMALL_BUFF * 0.6).set_color(
            v_color)
        ...
        p_label = VGroup(left.copy(), p_x, comma.copy(), p_y, right.copy()).arrange_submobjects(RIGHT,
                                                                                                buff=SMALL_BUFF * 0.6).set_color(
            p_color)
...
class TwoParticleDescription(Scene):
    ...
    def show_description(self):
        ...
        energy = TexMobject(
            r"\frac{1}{2} m_1v_1^2 + \frac{1}{2} m_2v_2^2 = \frac{1}{2} m_1 \hat{v}_1^2 + \frac{1}{2} m_2 \hat{v}_2^2")
        momentum.scale(0.8)
        energy.scale(0.8)
...
class SortAndSweep(GraphScene):
    ...
    def explain_sort_and_sweep(self, sort_and_sweep, particles, projections):
        ...
        self.play(
            *[ReplacementTransform(particle_list[i], sorted_particle_list[translations[i]]) for i in
              range(len(particles))],
            run_time=2,
        )
    ...
    def move_particle_to_active(self, particle, active_list, surround, sorted_list, original, sorted_particle_list,
                                projections):
        ...
        for i, other in enumerate(active_list):
            ...
            if current.get_left()[0] < active.get_right()[0]:
                ...
                if i != len(active_list) - 1:
                    self.play(
                        active_list[i + 1].move_to,
                        surround.get_left() + RIGHT * active_list[i + 1].radius + RIGHT * SMALL_BUFF,
                        run_time=2
                    )
...
class KDTree(Scene):
    ...
    def animate_all(self, steps, splits, split_labels, tree, edge_dict, grid_labels, particles):
        ...
        sorted_particles_list = [self.get_mobject_by_color(particles, color).copy() for color in
                                 [ORANGE, GREEN_SCREEN, PINK, RED, YELLOW, BLUE]]
...
class BVH(Scene):
    ...
    def get_bounding_volumes(self, particles):
        ...
        first_level_left_particles = VGroup(
            *[self.get_mobject_by_color(particles, color) for color in [PINK, BLUE, ORANGE]])
        first_level_right_particles = VGroup(
            *[self.get_mobject_by_color(particles, color) for color in [GREEN_SCREEN, RED, YELLOW]])
        ...
        second_level_bottom_left_particles = VGroup(
            *[self.get_mobject_by_color(particles, color) for color in [PINK, BLUE]])
        ...
        second_level_bottom_right_particles = VGroup(
            *[self.get_mobject_by_color(particles, color) for color in [RED, YELLOW]])
    ...
    def animate_all(self, steps, bounding_volumes, bounding_labels, tree, edge_dict, edge_set, particles):
        ...
        surround_highlight_1 = SurroundingRectangle(self.get_mobject_by_color(particles, BLUE), color=BLUE,
                                                    buff=SMALL_BUFF)
        surround_highlight_2 = SurroundingRectangle(self.get_mobject_by_color(particles, RED), color=RED,
                                                    buff=SMALL_BUFF)
        ...
        surround_highlight_1 = SurroundingRectangle(self.get_mobject_by_color(particles, PINK), color=PINK,
                                                    buff=SMALL_BUFF)
